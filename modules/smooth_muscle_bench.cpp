...
int smooth_muscle_bench(const Parameter *p_param)
{
  ...
  static const char *time_series_headers[] = {
    "Time(ms)", "v(mV)", "dv/dt(mV/ms)", "cai(nM)","Ist(pA_per_pF)",
    "ina(pA_per_pF)", "ical(pA_per_pF)", "icat(pA_per_pF)","ih(pA_per_pF)",
    "ik1(pA_per_pF)","ik2(pA_per_pF)","ika(pA_per_pF)","iBKa(pA_per_pF)","iBKab(pA_per_pF)",
    "icl(pA_per_pF)","insna(pA_per_pF)","insca(pA_per_pF)","insk(pA_per_pF)",
    "inak(pA_per_pF)","inaca(pA_per_pF)","ib(pA_per_pF)",
    "m(INa-activation)", "h(INa-inactivation)",
    "d(ICaL-activation)", "f1(ICaL-inactivation)", "f2(ICaL-inactivation-slow)",
    "b(ICaT-activation)", "g(ICaT-inactivation)",
    "w(Ih-activation)",
    "q(IK1-activation)", "r1(IK1-inactivation-fast)", "r2(IK2-inactivation-slow)",
    "p(IK2-activation)", "k1(IK2-inactivation-fast)", "k2(IK2-inactivation-slow)",
    "s(IKa-activation)", "x(IKa-inactivation)",
    "xa(IBKa-activation)", "xab(IBKab-beta-activation)",
    "y(ICaL-Ca2+-dependent-inactivation-fast)", "c(ICaL-Ca2+-dependent-inactivation-slow)"
  };
  ...
  while(tcurr < tmax){ // begin of computation loop
    ...
    if( tcurr >= next_output_time - cml::math::EPSILON ){
      ...
      double row_values[] = {
        ...
        p_cell->ALGEBRAIC[Ist],
        p_cell->ALGEBRAIC[ina],p_cell->ALGEBRAIC[ical],p_cell->ALGEBRAIC[icat],p_cell->ALGEBRAIC[ih],
        p_cell->ALGEBRAIC[ik1],p_cell->ALGEBRAIC[ik2],p_cell->ALGEBRAIC[ika],
        p_cell->ALGEBRAIC[iBKa],p_cell->ALGEBRAIC[iBKab],
        p_cell->ALGEBRAIC[icl],p_cell->ALGEBRAIC[insna],p_cell->ALGEBRAIC[insca],p_cell->ALGEBRAIC[insk],
        p_cell->ALGEBRAIC[inak],p_cell->ALGEBRAIC[inaca],p_cell->ALGEBRAIC[ib],
        p_cell->STATES[m], p_cell->STATES[h],
        p_cell->STATES[d],p_cell->STATES[f1],p_cell->STATES[f2],
        p_cell->STATES[b], p_cell->STATES[g],
        p_cell->STATES[w],
        p_cell->STATES[q],p_cell->STATES[r1],p_cell->STATES[r2],
        p_cell->STATES[p],p_cell->STATES[k1],p_cell->STATES[k2],
        p_cell->STATES[s], p_cell->STATES[x],
        p_cell->STATES[xa], p_cell->STATES[xab],
        p_cell->STATES[y], p_cell->STATES[c]
      };
